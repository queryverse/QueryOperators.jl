@testitem "Core" begin
    ...
    for i in QueryOperators.take(enum, 2)
        global taken_str *= string(i)
    end
    @test taken_str == "12"

    filtered_str = ""
    for i in QueryOperators.@filter(enum, x->x%2==0)
        global filtered_str *= string(i)
    end
    ...
    @test_throws ErrorException QueryOperators.@default_if_empty(source_1, "string")
